...
var Vitrine = Class.create(
{
  initialize: function(usuario, senha)
  {
    console.log("USUARIO:" + usuario + " SENHA: " + senha);
  ...
  attachClickProduct: function(sku, name, price, quantity){
      
      var item = 
      objAnalytics.model('item') //popular certinho
      .setSku('tenis_voador_pqp')
      .setName('Tenis voador')
      .setPrice(200.31)
      .setQuantity(1);
      console.log(item);
      window.open(
          'https://www.shibatacasa.com.br/mesa/luva-de-cozinha/luva-bordada-azulejo-30cm-azulbranco-christini-artefatti',
          '_blank' // <- This is what makes it open in a new window.
      );
  },

  setSwipperSlides: function(resposta, vitrinePrefs, vitrine)
  {
    let slides = [];
    resposta.forEach(atributo => {
        ...
        //     arr[vitrine.labels[1].attrName] = arr[vitrine.labels[1].attrName].substring(0, 99) + "...";
        // }
        var attachProduct = new Object();
        attachProduct.sku= 123;
        attachProduct.setName= "'batata'";
        attachProduct.setPrice=320.10;
        attachProduct.setQuantity=13;
        ...
        if (vitrine.grid.typeLayout === 1) {
            slides.push(
                ...
                '</div>\n' +
                '<div id="cardTexts">\n' +
                '<button onclick="vitrine.attachClickProduct('+attachProduct.sku + ',' + attachProduct.setName + ',' + attachProduct.setPrice + ',' + attachProduct.setQuantity +')">PRODUTO</button>' +
                // '<p class="text0">' + arr[vitrine.labels[0].attrName] + '</p>\n' +
                // '<p align="center" class="text1">' + arr[vitrine.labels[1].attrName] + '</p>\n' +
                // '<p class="text2">' + arr[vitrine.labels[2].attrName] + '</p>\n' +
                '<p class="text0">' + arr["id"] + '</p>\n' +
                '<p align="center" class="text1">' + arr["deion"] + '</p>\n' +
    ...
    this.configSwipper(slides, vitrinePrefs, vitrine);
  },

  configSwipper: function(slides, vitrinePrefs, vitrine) {
      vitrinePrefs.slidesPerView = vitrine.grid.narrow > slides.length ? slides.length : vitrine.grid.narrow;
